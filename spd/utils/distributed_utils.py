...
_state: DistributedState | NoDistribution = NoDistribution()


def get_distributed_state() -> DistributedState | NoDistribution:
    """Return the cached distributed state. If not initialized, returns NoDistribution.
...
def init_distributed(backend: Literal["nccl", "gloo"]) -> DistributedState | NoDistribution:
    ...
    """
    global _state
    assert isinstance(_state, NoDistribution), "Distributed state already initialized"
...
def get_rank() -> int | None:
    ...
    match get_distributed_state():
        case NoDistribution():
            return None
        case DistributedState(rank=rank):
            return rank
...
def get_local_rank() -> int | None:
    ...
    match get_distributed_state():
        case NoDistribution():
            return 0
        case DistributedState(local_rank=local_rank):
            return local_rank
...
def get_device() -> str:
    ...
    match get_distributed_state():
        case NoDistribution():
            return "cpu"
        case DistributedState(backend=backend, rank=rank):
            if backend == "gloo":
                return "cpu"
            return f"cuda:{rank}"
...
def gather_all_tensors(tensor: Tensor) -> list[Tensor]:
    ...
    if isinstance(state, NoDistribution):
        return [tensor]
