...
_state: DistributedState | NoDistribution | None = None


def get_distributed_state() -> DistributedState | NoDistribution:
    """Return the cached distributed state. If not initialized, raises an error.

    Returns:
        DistributedState: The current process's distributed state snapshot.
    """
    assert _state is not None, "Distributed state not initialized"
    return _state


def init_distributed(backend: Literal["nccl", "gloo"] | None) -> DistributedState | NoDistribution:
