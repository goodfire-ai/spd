...
def get_distributed_rand_like(
    ...
    # torch.rand(100_000_000, device="cuda") takes about 0.2ms
    ...
    out = torch.rand(*shape, generator=generator, device=device)
    torch.cuda.synchronize()
    end_time = time.perf_counter()
    print(f"Time to generate: {end_time - start_time:.3f}s")
    return out
