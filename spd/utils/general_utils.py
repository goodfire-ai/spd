...
def get_linear_ramp_value(
    ...
    """Generic linear ramp function.

    Returns a value that is constant at start_value before start_frac, linearly ramps to
...
def get_exp_ramp_value(
    ...
    """Exponential ramp between start_value and end_value."""   
    assert 0.0 <= start_frac <= 1.0 and 0.0 <= end_frac <= 1.0, (
        f"start_frac ({start_frac}) and end_frac ({end_frac}) must be within [0, 1]"
    )
    assert end_frac >= start_frac, f"end_frac ({end_frac}) must be >= start_frac ({start_frac})"
    ...
    progress = (cur_frac - start_frac) / (end_frac - start_frac)
    return float(start_value * np.exp(progress * np.log(end_value / start_value)))
