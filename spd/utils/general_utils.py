...
def with_parameter_noise(
    ...
    """Context manager that adds Gaussian noise to specified parameters during forward passes.
    
    The noise is permanently added to the parameters and affects both inference and training.
    Fresh noise is generated for each forward pass within the context.
    ...
    Example:
        with with_parameter_noise(model, 0.01, ["components"]):
            # Forward pass with noisy parameters (affects both inference and training)
            output = model(input)
    """
