...
def get_command(
    ...
    match n_gpus:
        ...
        case _:
            ...
            setup_and_run = f"{setup}\n{torchrun_cmd}"

            # --cpus-per-task=128 required because SLURM allocates 16 cpus per gpu (DefCpuPerGPU=16),
            # and srun defaults to 1 cpu per task. Must be 8 * 16 = 128 to avoid resource errors.
            command = f"srun --cpus-per-task=128 bash -c {shlex.quote(setup_and_run)}"
