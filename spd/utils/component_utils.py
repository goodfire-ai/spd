...
def calc_routing_masks(
    ...
    match routing:
        ...
        case float() | int() as pct_active:
            # interpolate up to min 1 so that we don't start with always off
            n_layers_active = _interpolate(1, len(module_names), pct_active)
            int_noise = torch.randint(
                low=1,
                high=len(module_names) + 1,
                size=(len(module_names), *leading_dims),
                device=device,
            )
            return {mod: int_noise[i] < n_layers_active for i, mod in enumerate(module_names)}
