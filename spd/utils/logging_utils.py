...
def get_grad_norms_dict(
    ...
    if component_model.is_global_ci:
        if component_model.global_ci_fn is not None:
            for local_param_name, local_param in component_model.global_ci_fn.named_parameters():
                ci_fn_grad = runtime_cast(Tensor, local_param.grad)
                ci_fn_grad_sum_sq = ci_fn_grad.pow(2).sum()
                key = f"ci_fns/global.{local_param_name}"
                assert key not in out, f"Key {key} already exists"
