...
def generate_clustering_commands(
    pipeline_config: ClusteringPipelineConfig, ensemble_id: str, run_dirs: dict[int, Path]
) -> list[str]:
    ...
    Args:
        ...
        run_dirs: Dictionary of (idx_in_ensemble, output_dir) pairs indicating where each run should
            save its output
    ...
    for idx, run_dir in run_dirs.items():
        command = (
...
def main(pipeline_config_path: Path, n_runs: int | None = None) -> None:
    ...
    run_dirs: dict[int, Path] = {}  # idx_in_ensemble -> output_dir
    for idx in range(pipeline_config.n_runs):
        outpath = pipeline_config.base_output_dir / "clustering_runs" / f"{ensemble_id}_{idx}"
        outpath.mkdir(parents=True, exist_ok=True)

        # Save the pipeline config for reference
        config_save_path = outpath / "pipeline_config.yaml"
        pipeline_config.to_file(config_save_path)
        run_dirs[idx] = outpath

    commands = generate_clustering_commands(
        pipeline_config=pipeline_config, ensemble_id=ensemble_id, run_dirs=run_dirs
    )
