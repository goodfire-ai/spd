...
def analyze_history(
    ...
    for module in sorted(module_components.keys()):
        ...
        print(
            f"  {module}: {len(components)} components (indices {min(components)}-{max(components)})"
        )
    print()
    ...
    sorted_cluster_ids = sorted(
        cluster_id_to_size.keys(), key=lambda x: cluster_id_to_size[x], reverse=True
    )
    ...
    for module in sorted(module_components.keys()):
        ...
        print(f"\n{module}:")
        print(
            f"  Components: {len(cluster_ids)} ({singletons} singletons, {in_multi} in multi-component clusters)"
        )
        ...
        for cid in sorted_cluster_ids[:top_clusters]:
            ...
            if count_in_cluster > 0:
                print(
                    f"  → In cluster {cid} (size {cluster_id_to_size[cid]}): {count_in_cluster} components"
                )
    ...
    if show_plots:
        ...
        largest_cluster_size = (
            cluster_id_to_size[sorted_cluster_ids[0]] if sorted_cluster_ids else 0
        )
        if largest_cluster_size > 1:
            ...
            axes[1, 1].plot(
                range(len(cluster_size_over_time)), cluster_size_over_time, "r-", linewidth=2
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze a clustering run from a history.zip file")
    parser.add_argument(
        "history_path",
        type=Path,
