...
def generate_clustering_commands(
    ...
    for idx in range(submit_config.n_runs):
        command = (
            f"echo 'getting the current python path' && "
            f"which python && "
            f"uv pip show spd && "
            f'python -c \'from pathlib import Path; import sys; p=Path("spd/clustering/run_clustering.py").resolve().parent; sys.path[0]=str(p); import spd; print("spd.__file__=", spd.__file__); print("sys.path[0]=", sys.path[0])\' && '
