...
def main(ensemble_id: str, clustering_dir: Path | None = None) -> None:
    if clustering_dir is None:
        clustering_dir = SPD_CACHE_DIR / "clustering"

    run_dirs = [i for i in clustering_dir.iterdir() if i.name == f"clustering_{ensemble_id}"]
    print(run_dirs)
