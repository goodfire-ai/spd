...
def _relabeled_partition(
    ...
    sizes = np.bincount(inv, minlength=int(inv.max()) + 1).astype(np.int64)
...
def _pairwise_m11(
    ...
    combo = inv_a * np.int64(k_b) + inv_b
    counts = np.bincount(combo).astype(np.int64)
...
def jaccard_partition_matrix(
    ...
    invs = []
    sizes_list = []
    sum_combs = np.zeros((k_rows,), dtype=np.int64)
    ...
    for i in range(1, k_rows):
        inv_i = invs[i]
        sum_comb_i = int(sum_combs[i])
        for j in range(i):
            inv_j = invs[j]
            sum_comb_j = int(sum_combs[j])
            m11 = _pairwise_m11(inv_i, inv_j, k_b=int(sizes_list[j].shape[0]))
            denom = sum_comb_i + sum_comb_j - m11
