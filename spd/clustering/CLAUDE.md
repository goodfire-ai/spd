...
2. Compute component activations:
   - **LM tasks**: Uses `n_tokens` parameter. Iterates through batches of size `batch_size`, picks one random token position per sequence, collects CI values until `n_tokens` samples gathered. Result: `(n_tokens, C)` per layer.
