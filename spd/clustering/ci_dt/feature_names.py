...
def generate_feature_names(
    ...
    """Generate feature names for all layers with activation and decoding info.

    Args:
        component_model: The ComponentModel containing components
        component_acts: Dictionary of continuous activations [n_samples, n_components]
        layers_true: List of boolean activations per layer
        layers_pred: List of predicted boolean activations per layer
        tokenizer: Tokenizer for decoding directions
        module_keys: List of module keys in order (matches layers_true)
        top_k: Number of top tokens to show for each direction

    Returns:
        List of feature name lists, one per layer. feature_names[k] contains
        names for all features used to predict layer k (concatenation of layers 0..k-1).
    """
    try:
        embed, unembed = get_embed_unembed_matrices(component_model)
    except ValueError as e:
        print(f"Warning: Could not extract embed/unembed matrices: {e}")
        embed = None
        unembed = None

    feature_names_per_layer: list[list[str]] = []

    # For each target layer k (k=1..L-1), we need feature names for concat(layers[:k])
    for k in range(1, len(layers_true)):
        feature_names_k: list[str] = []

        # Iterate through all previous layers (0..k-1)
        for layer_idx in range(k):
            module_key = module_keys[layer_idx]
            n_components = layers_true[layer_idx].shape[1]

            # Get continuous activations for this module
            module_acts = component_acts[module_key].numpy()  # [n_samples, n_total_components]

            # Map from filtered component indices to original component indices
            # layers_true[layer_idx] has only the varying components
            varying_mask = module_acts.var(axis=0) > 0
            varying_indices = np.where(varying_mask)[0]

            for filtered_idx in range(n_components):
                original_idx = varying_indices[filtered_idx]

                # Get actual and predicted activation stats
                actual_acts = module_acts[:, original_idx]
                actual_mean = float(actual_acts.mean())
                actual_std = float(actual_acts.std())

                # Component label
                comp_label = f"L{layer_idx}C{original_idx}"

                # Get activation info
                act_info = f"μ={actual_mean:.2f},σ={actual_std:.2f}"

                # Get direction decodings if available
                if embed is not None and unembed is not None:
                    try:
                        read_dir, write_dir = get_component_directions(
                            component_model, module_key, original_idx
                        )

                        # Decode using both embed and unembed
                        read_embed = decode_direction_top_k(
                            read_dir, embed, unembed, tokenizer, k=top_k, use_embed=True
                        )
                        write_unembed = decode_direction_top_k(
                            write_dir, embed, unembed, tokenizer, k=top_k, use_embed=False
                        )
                        ...
                        feature_name = f"{comp_label}\n{act_info}"
                else:
                    feature_name = f"{comp_label}\n{act_info}"

                feature_names_k.append(feature_name)

        feature_names_per_layer.append(feature_names_k)

    return feature_names_per_layer
