...
from spd.metrics.pgd_masked_recon_loss import PGDReconLoss, pgd_recon_loss
...
def run_experiment(
    ...
    for batch_size in batch_sizes:
        ...
        results[f"batch_size_{batch_size}"] = loss.item()

        print(
            f"{model_type}, {mask_scope}, seed={seed}, batch_size={batch_size}, loss={loss.item()}"
        )
...
def plot_results(
    results: dict[ModelType, dict[Literal["unit", "sign"], dict[str, dict[str, float]]]],
    output_path: Path,
    log_scale: bool = True,
) -> None:
    """Plot results with subplots for each model."""
    _, axes = plt.subplots(2 * 2, 2, figsize=(14, 10))
    axes = axes.flatten()

    model_names = ["tms_40_10", "tms_40_10_id", "resid_mlp1", "resid_mlp2"]
    step_types = ["unit", "sign"]
...
def main() -> None:
    ...
    # Collect results
    results: dict[ModelType, dict[Literal["unit", "sign"], dict[str, dict[str, float]]]] = {
        model_type: {} for model_type in model_configs
    }
    step_types: list[Literal["unit", "sign"]] = ["unit", "sign"]
    ...
    for step_type in step_types:
        for model_type, run_info_path in model_configs.items():
            ...
            for seed in seeds:
                ...
                results[model_type][step_type][f"seed_{seed}"] = seed_results
