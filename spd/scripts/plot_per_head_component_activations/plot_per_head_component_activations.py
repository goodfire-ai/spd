...
def _mean_abs_act_on_ci_data(
    comp_data: ComponentData, ci_threshold: float
) -> float:
    """Compute mean absolute component activation on CI-important positions."""
    acts: list[float] = []
    for example in comp_data.activation_examples:
        for ci, stored_act in zip(example.ci_values, example.component_acts):
            if ci > ci_threshold:
    ...
    with torch.no_grad():
        for layer_idx in range(n_layers):
            for proj_name in PROJ_NAMES:
                ...
                logger.info(
                    f"{module_path}: {len(alive)} components with mean_ci > {MIN_MEAN_CI}"
                )
