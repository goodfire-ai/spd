...
class ScaledOnehotAnalysisConfig(BaseModel):
    ...
    model_path: str = Field(
        default="./wandb/6hk3uciu/files/resid_mlp.pth", description="Path to the trained model (wandb:project/run_id or local path)"
    )
    model_type: str = Field(
        default="auto", description="Model type: 'spd', 'target', or 'auto' (default: auto)"
    )
    min_scale: float = Field(default=-2.0, description="Minimum scale for one-hot vectors (default: -2.0)")
    max_scale: float = Field(default=2.0, description="Maximum scale for one-hot vectors (default: 2.0)")
...
def compute_scaled_onehot_responses(
    ...
    for layer_name in layer_names:
        responses[layer_name] = torch.zeros(
            n_steps, n_features_to_plot, n_features, device=device
        )
...
def plot_scaled_onehot_responses(
    ...
    for layer_name, layer_responses in responses.items():
        ...
        responses_np = layer_responses.detach().cpu().numpy()  # Shape: (n_steps, n_features_to_plot, n_features)
...
def main(config_path_or_obj: str | ScaledOnehotAnalysisConfig | None = None) -> None:
    ...
    try:
        if config.model_type == "spd" or (config.model_type == "auto" and "wandb:" in config.model_path):
            # Load as SPD model
            run_info = SPDRunInfo.from_path(config.model_path)
            model = ComponentModel.from_run_info(run_info)
            model.to(device)
            model.eval()
            logger.info(f"Successfully loaded SPD model with {len(model.components)} component modules")
        ...
        elif config.model_type == "target" or (config.model_type == "auto" and "wandb:" not in config.model_path):
            ...
            raise ValueError(f"Invalid model_type: {config.model_type}. Must be 'spd', 'target', or 'auto'")
    ...
    if config.n_features_to_plot > n_features:
        logger.warning(f"n_features_to_plot {config.n_features_to_plot} > n_features {n_features}, using {n_features}")
