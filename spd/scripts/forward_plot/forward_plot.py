#!/usr/bin/env python3
"""
Forward plot script for SPD models.

This script generates forward pass plots for SPD models, showing how data flows through
the model components during inference.
...
class ForwardPlotConfig(BaseModel):
    """Configuration for forward plot script."""
...
def load_model(config: ForwardPlotConfig) -> ComponentModel:
...
def create_plot(_model: ComponentModel, output_path: Path, config: ForwardPlotConfig) -> None:
    """Create forward pass plot.
    ...
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_title("Forward Pass Plot")
...
def main(config_path_or_obj: str | ForwardPlotConfig) -> None:
    """Main function for forward plot."""
    config = load_config(config_path_or_obj, config_model=ForwardPlotConfig)
    ...
    output_path = output_dir_path / "forward_plot.png"
    create_plot(model, output_path, config)

    logger.info("Forward plotting complete!")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        config_path = Path(__file__).parent / "forward_plot_config.yaml"
