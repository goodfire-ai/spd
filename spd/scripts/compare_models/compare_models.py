...
class ModelComparator:
        ...
        with torch.no_grad():
            for batch in batches:
                ...
                for module_name, ci_vals_current in ci_current.items():
                    ...
                    ci_vals_reference_fp32 = ci_vals_reference.to(device=device, dtype=torch.float32)

                    ci_current_flat = ci_vals_current_fp32.reshape(-1, ci_vals_current_fp32.shape[-1])
                    ci_reference_flat = ci_vals_reference_fp32.reshape(
                        -1, ci_vals_reference_fp32.shape[-1]
                    )

                    ci_cross_dot_products[module_name] += ci_current_flat.transpose(0, 1) @ ci_reference_flat
            ...
            if layer_name in ci_cosine_similarities:
                ...
                if ci_cos_matrix.shape[0] != alive_mask.shape[0]:
                    ...
                    if ci_cos_alive.numel() > 0:
                        ...
                        similarities[f"ci_cosine_std/{layer_name}"] = ci_cos_max.std(unbiased=False).item()
    ...
    def run_comparison(
        ...
        similarities = self.compute_geometric_similarities(mean_component_cis, ci_cosine_similarities)
