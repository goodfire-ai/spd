...
Conditional inclusion - use null to exclude items from lists:
    ...
    loss_metric_configs:
      - classname: FaithfulnessLoss
        coeff: 1.0
      - __variants__:
          - classname: ImportanceMinimalityLoss  # included in variant 1
            coeff: 0.01
          - null                                  # excluded in variant 2
...
def expand_variants(obj: Any) -> list[Any]:
    ...
    Examples:
        ...
        # Conditional inclusion: null variants are filtered from lists
        expand_variants([{"a": 1}, {"__variants__": [{"b": 2}, null]}])
        # Returns: [[{"a": 1}, {"b": 2}], [{"a": 1}]]
    """
