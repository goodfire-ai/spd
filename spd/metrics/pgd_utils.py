...
from spd.utils.general_utils import calc_sum_recon_loss, map_dict_vals, zip_dicts
...
def pgd_masked_recon_loss_update(
    *,
    ...
    def objective_fn(
        ...
        total_loss, n_examples = calc_sum_recon_loss(
    ...
    *batch_dims, C = next(iter(ci.values())).shape
    ...
    def maybe_repeat_to_batch_shape(
        ...
        """abuse the fact that expand is a no-op for 1-element tensors"""
        return (
            map_dict_vals(component_sample_points, lambda x: x.expand(*batch_dims, -1)),
            map_dict_vals(weight_delta_mask, lambda x: x.expand(*batch_dims))
