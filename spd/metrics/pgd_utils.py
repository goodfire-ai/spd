    ...
    *batch_dims, C = next(iter(ci.values())).shape
    ...
    for layer in ci:
        source = call_on_rank0_then_broadcast(
            lambda: _get_pgd_init_tensor(pgd_config.init, ci_mask_shape, batch_device)
        ).requires_grad_(True)
    ...
    if weight_deltas is not None:
        ...
        for layer in ci:
            source = call_on_rank0_then_broadcast(
                lambda: _get_pgd_init_tensor(pgd_config.init, weight_delta_mask_shape, batch_device)
            ).requires_grad_(True)
            assert_same_across_ranks(source)
