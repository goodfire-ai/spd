...
def _stochastic_arb_hidden_acts_recon_loss_compute(
    ...
    return {f"recon_{layer_name}_{point}": (sum / n) for (layer_name, point), (sum, n) in results.items()}
...
class StochasticArbHiddenActsReconLoss(Metric):
    ...
    def __init__(
        ...
        self.sum_mse = defaultdict[CachePoint, Float[Tensor, ""]](lambda: torch.tensor(0.0, device=device))
