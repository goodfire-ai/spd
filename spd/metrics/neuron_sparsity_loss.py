...
def neuron_sparsity_loss(
    ...
    5. Average over batch and sequence dimensions (where appropriate)
    6. Sum over C

    The result is normalized by the number of batch/sequence elements.
    ...
    Returns:
        Scalar tensor with the total neuron sparsity loss (normalized)
    """
    ...
    for module_path in model.components:
        ...
        # Average over batch/seq dimensions (all except last), then sum over C
        # This is equivalent to: sum over all dims, then divide by n_batch_seq_elements
        n_batch_seq_elements = ci.shape[:-1].numel()
        module_loss = weighted.sum() / n_batch_seq_elements
