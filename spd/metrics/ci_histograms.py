...
class CIHistograms(Metric):
    ...
    def __init__(
        ...
        self.lower_leaky_hist_counts = defaultdict[str, Tensor](lambda: torch.zeros(self.N_BINS, device=device))
        self.pre_sigmoid_hist_counts = defaultdict[str, Tensor](lambda: torch.zeros(self.N_BINS, device=device))
