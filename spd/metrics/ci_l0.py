...
class CI_L0(MetricInterface):
    ...
    def __init__(
        ...
        self.l0_values: dict[str, list[Tensor]] = {key: [] for key in all_keys}
    ...
    def update(
        ...
        for layer_name, layer_ci in ci.items():
            ...
            self.l0_values[layer_name].append(torch.tensor([l0_val]))
        ...
        for group_name, group_sum in group_sums.items():
            self.l0_values[group_name].append(torch.tensor([group_sum]))
    ...
    def compute(self) -> dict[str, float | wandb.plot.CustomChart]:
        ...
        for key, l0s in self.l0_values.items():
            ...
            local_tensor = torch.cat(l0s, dim=0)
            gathered = gather_all_tensors(local_tensor)
            l0_tensor = torch.cat(gathered, dim=0)
