...
from spd.plotting import plot_causal_importance_vals, plot_multi_magnitude_causal_importance_vals
...
class MultiMagnitudeCIPlots(Metric):
    slow: ClassVar[bool] = True
    input_magnitudes: ClassVar[list[float]] = [-1.0, -0.75, -0.25, 0.0, 0.25, 0.75, 1.0]
    ...
    def compute(self) -> dict[str, Image.Image]:
        ...
        figures = plot_multi_magnitude_causal_importance_vals(
            model=self.model,
            batch_shape=self.batch_shape,
            input_magnitudes=self.input_magnitudes,
            sigmoid_type=self.sigmoid_type,
            identity_patterns=self.identity_patterns,
            dense_patterns=self.dense_patterns,
            sampling=self.sampling,
