...
class ComponentModel(LoadableModule):
    ...
    @contextmanager
    def _replaced_modules(self, masks: dict[str, Float[Tensor, "... C"]], r: dict[str, Tensor] | None = None):
        ...
        for module_name, component in self.components_or_modules.items():
            ...
            if module_name in masks or f"identity_{module_name}" in masks:
                ...
                if module_name in masks:
                    ...
                    # Set r for mixing at this module if provided, otherwise None
                    component.r = None if r is None else r.get(module_name, None)
                if f"identity_{module_name}" in masks:
                    ...
                    # Also ensure r is set if only identity_ mask was provided
                    if component.r is None:
                        component.r = None if r is None else r.get(module_name, None)
            else:
    ...
    def _forward_with_components(
        ...
        **kwargs: Any,
        ...
        """
        with self._replaced_modules(masks, r=r):
