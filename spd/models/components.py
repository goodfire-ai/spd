...
class VectorSharedMLPGate(nn.Module):
    ...
    def __init__(self, C: int, input_dim: int, hidden_dims: list[int]):
        ...
        self.C = C
        self.input_dim = input_dim
        assert C % 4 == 0, "C must be divisible by 4 for 4-head attention"

        # Initial projection to component dimension followed by RMSNorm
        self.input_proj = Linear(input_dim, C, nonlinearity="linear")
        self.input_norm = nn.RMSNorm(C)

        # Stack of transformer-style blocks, each keeping embed dim = C and FF width = hidden_dims[i]
        self.blocks = nn.ModuleList(
            [self._TransformerGateBlock(embed_dim=C, ff_hidden_dim=h) for h in hidden_dims]
        )

    @override
    def forward(self, x: Float[Tensor, "B S d_in"]) -> Float[Tensor, "B S C"]:
        # Project to component dimension and normalize (no reshaping needed)
        x_proj = self.input_proj(x)
        x_proj = self.input_norm(x_proj)

        # Transformer blocks over existing sequence dimension
        x_seq = x_proj
