...
def plot_training_metrics(
    ...
    ax_loss.plot(teacher_metrics.steps, teacher_metrics.train_losses, 
                'b-', label='Teacher Train', linewidth=2, alpha=0.8)
    ax_loss.plot(teacher_metrics.steps, teacher_metrics.val_losses, 
                'b--', label='Teacher Val', linewidth=2, alpha=0.8)
    ax_loss.plot(student_metrics.steps, student_metrics.train_losses, 
                'r-', label='Student Train', linewidth=2, alpha=0.8)
    ax_loss.plot(student_metrics.steps, student_metrics.val_losses, 
                'r--', label='Student Val', linewidth=2, alpha=0.8)
...
def plot_output_distributions(
    ...
    def collect_outputs(loader: DataLoader, tag: str) -> tuple[Float[Tensor, "total 10"], Float[Tensor, "total aux"]]:
    ...
    for i in range(num_aux):
        ...
        teacher_mnist_aux_counts, _ = np.histogram(
            mnist_aux_probs[:len(mnist_aux_probs)//2, i].numpy(), 
            bins=bin_edges, density=True
        )
        ...
        teacher_noise_aux_counts, _ = np.histogram(
            noise_aux_probs[:len(noise_aux_probs)//2, i].numpy(), 
            bins=bin_edges, density=True
        )
