...
class MemDataset(
    ...
    def __init__(
        ...
        self.fact_labels = torch.randint(
            0, vocab_size, (n_facts,), generator=gen, dtype=torch.long
        )
    ...
    def __getitem__(
        self, idx: int
    ) -> tuple[Float[Tensor, "seq_len"], Float[Tensor, ""]]:
        """Get a single fact by index."""
        return self.fact_inputs[idx], self.fact_labels[idx]

    def generate_batch(
        self, batch_size: int
    ) -> tuple[Float[Tensor, "batch seq_len"], Float[Tensor, "batch"]]:
    ...
    def get_all_facts(
        self,
    ) -> tuple[Float[Tensor, "n_facts seq_len"], Float[Tensor, "n_facts"]]:
