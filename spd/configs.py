...
class Config(BaseModel):
    ...
    def validate_model(cls, values: dict) -> dict:
        ...
        if values.get("lr") is not None and (values.get("gate_lr") is None or values.get("component_lr") is None):
            if values.get("gate_lr") is None:
                values["gate_lr"] = values["lr"]
            if values.get("component_lr") is None:
                values["component_lr"] = values["lr"]
        
        # Handle backward compatibility for schedules: if old schedule is set but new ones are default, use old values
        if (values.get("lr_schedule") != "constant" and 
            values.get("gate_lr_schedule") == "constant" and 
            values.get("component_lr_schedule") == "constant"):
            values["gate_lr_schedule"] = values["lr_schedule"]
            values["component_lr_schedule"] = values["lr_schedule"]
        
        if (values.get("lr_exponential_halflife") is not None and 
            values.get("gate_lr_exponential_halflife") is None and 
            values.get("component_lr_exponential_halflife") is None):
            values["gate_lr_exponential_halflife"] = values["lr_exponential_halflife"]
            values["component_lr_exponential_halflife"] = values["lr_exponential_halflife"]
        
        if (values.get("lr_warmup_pct") != 0.0 and 
            values.get("gate_lr_warmup_pct") == 0.0 and 
            values.get("component_lr_warmup_pct") == 0.0):
            values["gate_lr_warmup_pct"] = values["lr_warmup_pct"]
            values["component_lr_warmup_pct"] = values["lr_warmup_pct"]
