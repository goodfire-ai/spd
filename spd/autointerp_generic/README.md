...
## Future: abstract harvest over methods too

The harvest pipeline (`spd/harvest/`) could also be made generic. The only method-specific part is `batch -> {component_key: per_token_activations}`. Everything else — collecting top examples, computing token PMI, correlations, stats — is the same regardless of decomposition method. This would mean callers only need to provide a single activation function to get the full harvest + autointerp + evals pipeline.
