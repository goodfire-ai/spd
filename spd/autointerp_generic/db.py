"""SQLite database for generic autointerp data (interpretations + scores)."""
...
from spd.autointerp_generic.types import InterpretationResult
...
class GenericInterpDB:
    ...
    def get_labels(self) -> dict[str, str]:
        rows = self._conn.execute("SELECT component_key, label FROM interpretations").fetchall()
        return {row["component_key"]: row["label"] for row in rows}
