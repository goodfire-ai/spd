...
def calc_stochastic_recon_subset_loss(
    ...
    for _ in range(n_subset_samples):
        ...
        # Create CI subset for selected modules
        ci_subset = {
            module_name: causal_importances[module_name] for module_name in modules_to_replace
        }

        # Generate stochastic masks for selected modules
        stoch_masks_list, r = calc_stochastic_masks(
            causal_importances=ci_subset, n_mask_samples=n_mask_samples, sampling=sampling
        )

        # Calculate loss for each mask sample
        subset_loss = torch.tensor(0.0, device=device)
        for stoch_mask in stoch_masks_list:
            # Forward pass with subset masks and r-parameter
            out = model(batch, mode="components", masks=stoch_mask, r=r)
