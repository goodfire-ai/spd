...
def calc_masked_recon_layerwise_loss(
    ...
    for mask_info in masks:
        for comp_name, mask in mask_info.items():
            ...
            # For layerwise eval here, r is not used; pass None
            modified_out = model(batch, mode="components", masks={comp_name: mask}, r=None)
            if loss_type == "mse":
...
def calculate_losses(
    ...
    if config.stochastic_recon_layerwise_coeff is not None:
        layerwise_stochastic_masks, _ = calc_stochastic_masks(
            causal_importances=causal_importances,
            n_mask_samples=config.n_mask_samples,
            sampling=config.sampling,
        )
