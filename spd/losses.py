...
def _optimize_adversarial_stochastic_masks(
    *,
    ...
    # rand_tensors_opt: per-layer shape (1, ..., 1, C) to be broadcasted to (... C)
    # weight_delta_rand_masks_opt: per-layer shape (1, ..., 1) to be broadcasted to leading dims
    ...
    for layer, ci in causal_importances.items():
        ...
        # Create shape (1, 1, ..., 1, C) that can broadcast to target shape
        opt_shape = (1,) * len(leading_dims) + (num_components,)
        init = (
            torch.rand(opt_shape, device=ci.device, dtype=ci.dtype)
            if config.pgd_mask_random_init
            else torch.zeros(opt_shape, device=ci.device, dtype=ci.dtype)
        )
        init.requires_grad_(True)
        rand_tensors_opt[layer] = init
        
        if weight_delta_rand_masks_opt is not None:
            # Create shape (1, 1, ..., 1) that can broadcast to leading dims
            wd_opt_shape = (1,) * len(leading_dims)
            wd_init = (
                torch.rand(wd_opt_shape, device=ci.device, dtype=ci.dtype)
                if config.pgd_mask_random_init
                else torch.zeros(wd_opt_shape, device=ci.device, dtype=ci.dtype)
            )
    ...
    try:
        ...
        for _ in range(int(config.pgd_mask_steps)):
            ...
            # Use optimized variables directly - they will broadcast automatically
            # Broadcasting is more efficient than explicit expand
            ...
            if config.stochastic_recon_coeff is not None:
                mask_infos = calc_stochastic_component_mask_info(
                    ...
                    rand_tensors=rand_tensors_opt,
                    weight_delta_rand_mask=weight_delta_rand_masks_opt,
                )
    ...
    # Broadcast optimized variables to full shapes for return
    ...
    with torch.no_grad():
        for layer, ci in causal_importances.items():
            # Detach, clamp, and broadcast rand_tensors to full shape (... C)
            target_shape = ci.shape
            rand_tensors[layer] = (rand_tensors_opt[layer].detach().clamp_(0.0, 1.0) 
                                   + torch.zeros(target_shape, device=ci.device, dtype=ci.dtype))
            
            # Detach, clamp, and broadcast weight_delta_rand_masks to leading dims
            if weight_delta_rand_masks is not None and weight_delta_rand_masks_opt is not None:
                weight_delta_rand_masks[layer] = (
                    weight_delta_rand_masks_opt[layer].detach().clamp_(0.0, 1.0)
                    + torch.zeros(leading_dims, device=ci.device, dtype=ci.dtype)
                )
