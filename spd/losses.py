...
def calc_masked_recon_loss(
    ...
    assert output_recon_loss_type in ["mse", "kl"], f"Invalid output loss type: {output_recon_loss_type}"
    ...
    for mask_infos in mask_infos_list:
        ...
        if return_hidden_act_recon_losses and target_hidden is not None:
            for layer_name, target_activation in target_hidden.items():
                assert layer_name in component_input_acts, f"Layer {layer_name} not found in component_input_acts"
...
def calculate_losses(
    ...
    if config.stochastic_recon_coeff is not None:
        ...
        compute_hidden_losses = (config.hidden_act_recon_coeff is not None and target_hidden is not None)
        ...
        if compute_hidden_losses:
            ...
            assert config.hidden_act_recon_coeff is not None, "hidden_act_recon_coeff should not be None when computing hidden losses"
