...
def calc_importance_minimality_loss(
    ...
    # Experimental calc
    for layer_ci_upper_leaky in ci_upper_leaky.values():
        # Where the layer_ci_upper_leaky is > 1, we use pnorm 2, otherwise we use 0.9
        above_one = layer_ci_upper_leaky > 1
        higher_pnorm = (layer_ci_upper_leaky + eps) ** 2.0
        lower_pnorm = (layer_ci_upper_leaky + eps) ** 0.9
