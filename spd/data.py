    ...
    if dataset_config.is_tokenized:
        ...
        if dataset_config.n_ctx < tokenized_len:
            ...
            if isinstance(torch_dataset, Dataset):
                # set_transform replaces with_format("torch"), so we convert to tensors here
                torch_dataset.set_transform(
                    lambda x: {col: torch.tensor([row[:n_ctx] for row in x[col]])}
                )
            else:
