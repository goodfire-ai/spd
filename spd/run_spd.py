        ...
        for _ in range(config.gradient_accumulation_steps):
            ...
            for loss_cfg, loss_val in microbatch_losses.items():
                ...
                if isinstance(
                    ...
                    ppgd_grads, r1_penalty = ppgd_states[loss_cfg].get_grads(
                    ...
                    if r1_penalty is not None:
                        microbatch_total_loss = (
                            microbatch_total_loss + r1_penalty / config.gradient_accumulation_steps
                        )
                        batch_log_data[f"train/loss/{loss_cfg.classname}/r1_penalty"] += (
                            r1_penalty.item() / config.gradient_accumulation_steps
                        )
