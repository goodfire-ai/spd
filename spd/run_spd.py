...
def optimize(
    ...
    loss_configs: list[LossMetricConfigType] = config.loss_metric_configs
    ...
    for step in tqdm(range(config.steps + 1), ncols=0, disable=not is_main_process()):
        ...
        step_ppgd_grads: list[dict[str, Float[Tensor, " mask_c"]]] = []
        ...
        for _ in range(config.gradient_accumulation_steps):
            ...
            if persistent_pgd_configs:
                ...
                persistent_pgd_state.zero_grad()
                ...
                step_ppgd_grads.append(ppgd_grads)
                ...
                if is_main_process():
                    try_wandb(
                        wandb.log,
                        {
                            ...
                            for module_name, v in ppgd_grads.items()
                        },
                    )
        ...
        if step_ppgd_grads:
            assert persistent_pgd_state is not None, "PersistentPGD state is required"
            total_ppgd_grads = defaultdict[str, Float[Tensor, " mask_c"]](lambda: torch.zeros((), device=device))
            for key in list(step_ppgd_grads[0].keys()):
                # todo maybe do the all_reduce here?
                for grads in step_ppgd_grads:
                    total_ppgd_grads[key] += grads[key]
            persistent_pgd_state.step(total_ppgd_grads)
