...
class AttributionHarvester:
    ...
    def _accumulate_attributions(
        ...
        for source_layer, act, grad, abs_grad in zip(
            ...
            if source_layer == self.embed_path:
                # BENCHMARK: slow per-token loop (replaces scatter_add_)
                ...
                for i, tok_id in enumerate(tokens.flatten().tolist()):
                    attr_acc[tok_id] += attr[i]
                    attr_abs_acc[tok_id] += attr_abs[i]
                    square_attr_acc[tok_id] += attr_squared[i]

                # # FAST: scatter_add_ (production)
                # attr = ci_weighted_attr.sum(dim=-1).flatten()
                # attr_abs = ci_weighted_attr_abs.sum(dim=-1).flatten()
                # attr_squared = ci_weighted_squared_attr.sum(dim=-1).flatten()
                ...
                # BENCHMARK: slow per-component loop (replaces vectorized .add_)
                attr_summed = ci_weighted_attr.sum(dim=(0, 1))
                attr_abs_summed = ci_weighted_attr_abs.sum(dim=(0, 1))
                attr_squared_summed = ci_weighted_squared_attr.sum(dim=(0, 1))
                for c in range(attr_summed.shape[0]):
                    attr_acc[c] += attr_summed[c]
                    attr_abs_acc[c] += attr_abs_summed[c]
                    square_attr_acc[c] += attr_squared_summed[c]

                # # FAST: vectorized (production)
