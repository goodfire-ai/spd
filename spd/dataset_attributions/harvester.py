...
class AttributionHarvester:
    ...
    def _log_mem(label: str) -> None:
        ...
        logger.info(f"[MEM] {label}: {alloc:.1f}G allocated, {reserved:.1f}G reserved")
    ...
    def process_batch(self, tokens: Int[Tensor, "batch seq"]) -> None:
        ...
        if self.n_batches == 1:
            self._log_mem("after cache setup")

        for i, target_layer in enumerate(self.sources_by_target):
            ...
            if self.n_batches == 1:
                self._log_mem(f"after target {i}/{len(self.sources_by_target)} ({target_layer})")
    ...
    def _process_output_targets(
        ...
        for d_idx in range(self.output_d_model):
            ...
            with torch.no_grad():
                for source_layer, act, grad in zip(
                    source_layers, source_acts, grads, strict=True
                ):
    ...
    def _process_component_targets(
        ...
        alive_indices = torch.where(alive_targets)[0].tolist()
        for i, t_idx in enumerate(alive_indices):
            if self.n_batches == 1 and i % 100 == 0:
                self._log_mem(f"  component {i}/{len(alive_indices)} of {target_layer}")
