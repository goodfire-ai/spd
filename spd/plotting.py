...
def plot_single_layer_component_co_activation_fractions(
    ...
    norm = plt.Normalize(vmin=0, vmax=1)
    im = ax.matshow(component_co_activation_fractions.detach().cpu().numpy(), aspect="auto", cmap="Blues", norm=norm)
...
def plot_single_layer_geometric_interaction_strength_vs_coactivation(
    ...
    gs = fig.add_gridspec(2, 2, width_ratios=[1, 10], height_ratios=[10, 1], wspace=0.1, hspace=0.1)
    ...
    ax.hist(coactivation_fraction, bins=250, orientation="horizontal")
    ax.set_ylim(-0.01, 1.01)
    ax.set_title(f"{module_name} - Coactivation Fraction")
    ax.set_ylabel("Coactivation Fraction")
