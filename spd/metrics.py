...
def target_ci_solution(
    inputs: CreateMetricsInputs, evals_id: str
) -> Mapping[str, float | int | wandb.Table]:
    """Compute target solution metrics for toy models with canonical solutions.

    Args:
        inputs: Standard metrics inputs
        evals_id: Experiment ID to look up target solution

    Returns:
        Dictionary of target solution metrics
    """
    if not has_ci_solution(evals_id):
        return {}
    ...
    target_solution = TARGET_CI_SOLUTIONS[evals_id]
