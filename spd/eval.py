...
class CIMeanPerComponent(StreamingEval):
    ...
    def compute(self) -> Mapping[str, Image.Image]:
        ...
        mean_component_cis = {}
        for module_name in self.model.components:
            print(f"Module {module_name} has {self.component_ci_sums[module_name]} on this rank")
            ...
            print(
                f"Module {module_name} has {self.samples_seen[module_name]} on this rank and {all_samples_seen[module_name]} total"
            )
            print(f"Module {module_name} has {all_component_cis} total")
