...
class GeometricSimilarityComparison(StreamingEval):
    ...
    def _compute_subcomponent_geometric_similarities(self, activation_densities: dict[str, Float[Tensor, " C"]]) -> dict[str, float]:
        ...
        for layer_name in self.model.components:
            ...
            current_rank_one = einops.einsum(current_V, current_U, "d_in C_alive, C_alive d_out -> C_alive d_in d_out")
            ...
            cosine_sim_matrix = einops.einsum(current_norm, ref_norm, "C_alive d_in_d_out, C_ref d_in_d_out -> C_alive C_ref")
        ...
        similarities["mean_max_cosine_sim"] = sum(similarities.values()) / len(similarities)
        similarities["max_cosine_sim_std"] = torch.std(torch.tensor(list(similarities.values()))).item()
        similarities["max_cosine_sim_min"] = min(similarities.values())
        similarities["max_cosine_sim_max"] = max(similarities.values())
