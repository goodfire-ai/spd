...
class CEandKLLosses(StreamingEval):
    ...
    def _calc_ce_and_kl_losses(
        ...
        ci_masked_logits = self.model(batch, mode="components", masks=ci, r=None)
        ...
        stoch_masks, r = calc_stochastic_masks(ci, n_mask_samples=1, sampling=self.config.sampling)
        stoch_masked_logits = self.model(batch, mode="components", masks=stoch_masks[0], r=r)
        ...
        unmasked_logits = self.model(batch, mode="components", masks=nonmask, r=None)
        ...
        random_masked_logits = self.model(batch, mode="components", masks=rand_masks, r=None)
        ...
        rounded_masked_logits = self.model(batch, mode="components", masks=rounded_ci, r=None)
        ...
        zero_masked_logits = self.model(batch, mode="components", masks=zero_masks, r=None)
...
class StochasticReconLayerwiseLoss(StreamingEval):
    ...
    def watch_batch(
        ...
        # Calculate stochastic masks (returns masks and r for r-parameter approach)
        ...
        # Calculate loss using r-parameter approach
...
class SubsetReconstructionLoss(StreamingEval):
    ...
    def _calc_subset_losses(
        ...
        zero_masks = {k: torch.zeros_like(v) for k, v in ci.items()}
        zero_out = self.model(batch, mode="components", masks=zero_masks, r=None)
        zero_ce = ce_vs_labels(zero_out)

        # Generate stochastic masks
        stoch_masks, r = calc_stochastic_masks(ci, self.n_mask_samples, self.config.sampling)
