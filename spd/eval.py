...
class CEandKLLosses(StreamingEval):
    ...
    def _calc_ce_and_kl_losses(
        ...
        stoch_masks, rs = calc_stochastic_masks(ci, n_mask_samples=1, sampling=self.config.sampling)
        stoch_masked_logits = self.model(batch, mode="components", masks=stoch_masks[0], r=rs[0])
