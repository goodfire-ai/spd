"""V7: V5 without the top-k heap - collect all examples then filter at the end."""
...
def get_topk_by_subcomponent_v7(
    ...
    # for each (module_name, component_idx), just collect ALL examples (no heap)
    examples = defaultdict[str, defaultdict[int, list[SubcomponentExample]]](
    ...
    for _ in tqdm(range(n_batches), desc="Harvesting activation contexts"):
        ...
        for module_name, causal_importances in importances_by_module.items():
            ...
            # Now iterate to create examples and just append them all (no heap filtering)
            for j in range(K):
                ...
                # Just append - no heap logic
                examples[module_name][m].append(ex)
    ...
    # Now filter down to top-k examples by importance for each (module, component)
    topk_examples = defaultdict[str, defaultdict[int, list[SubcomponentExample]]](
        lambda: defaultdict(list)
    )
    for module_name, components in examples.items():
        for component_idx, example_list in components.items():
            # Sort by last_tok_importance descending and take top k
            sorted_examples = sorted(
                example_list, key=lambda ex: ex.last_tok_importance, reverse=True
            )
            topk_examples[module_name][component_idx] = sorted_examples[:TOPK_EXAMPLES]

    return ActivationsData(
        examples=topk_examples,
