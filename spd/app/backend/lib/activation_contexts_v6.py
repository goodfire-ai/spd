"""V6: Pad batch once per batch iteration, not per module."""
...
def get_topk_by_subcomponent_v6(
    ...
    # Pre-compute window offsets once for all batches
    window_size = 2 * n_tokens_either_side + 1
    window_offsets = None  # Will be created on first batch with correct device
    ...
    for _ in tqdm(range(n_batches), desc="Harvesting activation contexts"):
        ...
        # Create window offsets once on first iteration
