...
def parse_analysis_file(filepath: str):
    ...
    while i < len(lines):
        ...
        if current_section == 'component_analysis' and current_module:
            ...
            fact_match = re.match(r'Fact\s+(\d+): input=\[(\d+), (\d+), (\d+)\] → label=(\d+)', line)
            if fact_match and current_component is not None:
                fact_idx = int(fact_match.group(1))
                inputs = [int(fact_match.group(2)), int(fact_match.group(3)), int(fact_match.group(4))]
        ...
        if current_section == 'per_fact' and current_module:
            ...
            if fact_match:
                ...
                while j < len(lines):
                    ...
                    if next_line.startswith('Fact ') or next_line.startswith('===') or next_line.startswith('MODULE:'):
                        break
...
def score_fact(fact_info: dict, up_proj_mono_scores: dict, down_proj_mono_scores: dict,
               up_proj_stats: dict, down_proj_stats: dict) -> tuple[float, dict]:
    """
    ...
    if not up_components and not down_components:
        return 0.0, {'reason': 'no_components', 'up_proj_components': [], 'down_proj_components': [], 'n_components': 0}
    ...
    for comp_id, ci_score in up_components:
        ...
        is_mono, reason = is_component_monosemantic(stats, threshold=0.9) if stats else (False, "unknown")
    ...
    if not all_mono_scores:
        return 0.0, {'reason': 'no_scores', 'up_proj_components': up_scores, 'down_proj_components': down_scores, 'n_components': 0}
...
def main():
    ...
    up_proj_comps, down_proj_comps, up_proj_facts, down_proj_facts = parse_analysis_file('analysis.txt')
    ...
    for comp_id, stats, reason in mono_up[:20]:
        print(f"  Component {comp_id}: {reason}, score={compute_monosemanticity_score(stats):.3f}, n_facts={stats['n_facts']}")
    ...
    for fact_idx in all_facts:
        ...
        score, details = score_fact(combined_info, up_proj_mono_scores, down_proj_mono_scores,
                                    up_proj_stats, down_proj_stats)
    ...
    single_comp_facts = [
        fs for fs in fact_scores 
        if len(fs['details'].get('up_proj_components', [])) == 1
    ]
    ...
    for i, fs in enumerate(single_comp_facts[:30]):
        ...
        is_mono, reason = is_component_monosemantic(comp_stats, threshold=0.9) if comp_stats else (False, "unknown")
        
        down_str = ", ".join([f"C{c[0]}({c[4]})" for c in down_comps])
        
        print(f"{i+1}. Fact {fs['fact_idx']}: input={fs['inputs']} → label={fs['label']}")
        print(f"   Up_proj C{comp_id}: mono_score={fs['score']:.3f}, is_mono={is_mono}, reason={reason}")
        print(f"   Down_proj: {down_str if down_str else 'none'}")
        if comp_stats:
            print(f"   Component stats: dominant_label={comp_stats['dominant_label']} ({comp_stats['dominant_label_ratio']:.1%})")
        print()
    ...
    with open('clean_facts_ranking.txt', 'w') as f:
        ...
        for i, fs in enumerate(all_mono_facts):
            ...
            for comp_id, mono_score, ci_score, is_mono, reason in up_comps:
                f.write(f"    C{comp_id}: CI={ci_score:.3f}, mono={mono_score:.3f}, reason={reason}\n")
        ...
        for i, fs in enumerate(facts_with_components):
            ...
            for comp_id, mono_score, ci_score, is_mono, reason in up_comps:
                ...
                f.write(f"    {mono_marker} C{comp_id}: CI={ci_score:.3f}, mono={mono_score:.3f}, reason={reason}\n")
            f.write(f"  Down_proj components ({len(down_comps)}):\n")
            for comp_id, mono_score, ci_score, is_mono, reason in down_comps:
                mono_marker = "✓" if is_mono else "✗"
                f.write(f"    {mono_marker} C{comp_id}: CI={ci_score:.3f}, mono={mono_score:.3f}, reason={reason}\n")
