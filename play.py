...
def main(model_size, n_iters):
    ...
    t0 = benchmark.Timer(
        stmt="dist.all_reduce(mat)", setup="from __main__ import dist", globals={"mat": mat}
    )
    ...
    if local_rank == 0:
        print(
            f"Average time per iteration: {measured_time:.2f} seconds, Average bandwidth: {bandwidth:.2f} GB/s"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_size", type=int, default=int(1e9), help="Model size for the test matrix."
    )
