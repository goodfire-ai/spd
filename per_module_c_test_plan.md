...
### Test 8: Old format with global C auto-migrates
- **Description**: Using old `C: 20` + `target_module_patterns: ["linear1"]` should auto-migrate with deprecation warning
- **Input**: Config with both `C` and `target_module_patterns` as list of strings
- **Expected**: Auto-converts to `[["linear1", 20]]` format with deprecation log message
- **passes**: true
...
- **Description**: calc_weight_deltas should work with per-module C (returns summed deltas)
- **Input**: ComponentModel with varying C
- **Expected**: weight_deltas dict works correctly, shapes match original weight shapes
- **passes**: true
...
|----------|-------|--------|--------|
| Pattern Resolution | 7 | 7 | 0 |
| Config Validation | 5 | 5 | 0 |
| ComponentModel Integration | 6 | 6 | 0 |
| Full Training Loop | 3 | 3 | 0 |
| Identity Patterns | 3 | 3 | 0 |
| End-to-End | 3 | 3 | 0 |
| Regression | 3 | 3 | 0 |
| **TOTAL** | **30** | **30** | **0** |

**Feature Status**: READY - 30/30 tests passing
