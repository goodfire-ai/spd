...
def diagnose_run(run_id: str, timestamp: str = "20260218_000000") -> RunDiagnostic:
    ...
    if intruder_dir.exists():
        ...
        if jsonl_files:
            ...
            diag.intruder_scores = sum(1 for _ in open(largest, "rb"))
        ...
        if "scores" in tables:
            diag.interp_score_types = [
                r[0] for r in conn.execute(
                    "SELECT DISTINCT score_type FROM scores"
                ).fetchall()
            ]
        conn.close()
...
def print_diagnostics(diagnostics: list[RunDiagnostic]) -> None:
    ...
    if ready:
        ...
        print(f"{'run_id':20s} {'comps':>6s} {'n_tokens':>14s} {'threshold':>10s} "
              f"{'intruder':>8s} {'interps':>7s} {'scores':>20s} {'corr':>5s}")
        print("-" * 95)
        for d in ready:
            scores_str = ", ".join(d.interp_score_types) if d.interp_score_types else "-"
            print(f"{d.run_id:20s} {d.n_components:>6d} {d.n_tokens:>14,} {d.ci_threshold:>10s} "
...
def migrate_harvest_db(old_db_path: Path, new_db_path: Path, token_stats_path: Path) -> int:
    ...
    for row in rows:
        ...
        assert component_key in firing_density_map, (
            f"{component_key} missing from token_stats.pt"
        )
...
def _find_legacy_run_ids() -> list[str]:
    ...
    return sorted(
        d.name for d in harvest_root.iterdir()
