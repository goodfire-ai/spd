...
PROMPTS = [
    ...
    "Q: What comes next in the sequence 1, 2, 4, 8, 16?\nA:",
    # Pile-distinctive (arxiv, stack exchange, github, books)
    "\\begin{theorem}",
    "$ E = mc^",
...
def non_emb_params(model: torch.nn.Module, *emb_attrs: str) -> int:
...
def main():
    ...
    HF_MODELS: dict[str, tuple[str, tuple[str, ...]]] = {
        "gpt2-small": ("gpt2", ("transformer.wte",)),
        "pythia-70m": ("EleutherAI/pythia-70m", ("gpt_neox.embed_in", "embed_out")),
        "pythia-410m": ("EleutherAI/pythia-410m", ("gpt_neox.embed_in", "embed_out")),
    }
    for name, (hf_id, _emb_attrs) in HF_MODELS.items():
        model, tokenizer = load_hf_model(name, hf_id)
        models[name] = (model, tokenizer, False)

    # Compute param counts
    model_param_counts = {}
    for name, _ in CUSTOM_MODELS.items():
        model_param_counts[name] = non_emb_params(models[name][0], "wte")
    for name, (_, emb_attrs) in HF_MODELS.items():
        model_param_counts[name] = non_emb_params(models[name][0], *emb_attrs)
