"""Compare cross-entropy loss of pile-trained models, GPT-2 small, and Pythia models on the pile
validation set.
...
Usage:
    python scripts/compare_pile_ce_loss.py
    python scripts/compare_pile_ce_loss.py --val-max-steps 50
    python scripts/compare_pile_ce_loss.py --device cpu
"""
...
def make_val_config(hf_tokenizer_path: str) -> DatasetConfig:
    """Matches the validation config in pile_llama_simple_mlp-2L-768.yaml."""
...
def main():
    parser = argparse.ArgumentParser(description="Compare CE loss on pile validation set")
    parser.add_argument(
