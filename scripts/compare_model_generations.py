...
def load_models():
    ...
    models["gpt2_simple-pile (7M)"] = {
        ...
        "lowercase": True,
        "custom": True,
        "params": sum(p.numel() for p in model_2.parameters()),
    }
    ...
    models["pythia-70m"] = {
        ...
        "lowercase": False,
        "custom": False,
        "params": sum(p.numel() for p in pythia_70m.parameters()),
    }
    ...
    models["pythia-160m"] = {
        ...
        "lowercase": False,
        "custom": False,
        "params": sum(p.numel() for p in pythia_160m.parameters()),
    }
...
def format_markdown(results, models, args):
    ...
    for name, config in models.items():
        lines.append(f"| {name} | {config['params']:,} |")
