...
def main():
    ...
    for split in SPLITS:
        ...
        # num_proc affects both the .map() cache fingerprint and the shard count.
        # tokenize_and_concatenate overrides the fingerprint to exclude num_proc, but the
        # shard count is still embedded in cache filenames. Training code must use the same
        # num_proc (or symlink the cache) for the cache to be found.
