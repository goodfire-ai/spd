window.initCgLinkGraph = function({visState, renderAll, data, cgSel}){
  ...
  var yNumTicks= visState.isHideLayer ? data.byStream.length : d3.max(nodes, d => d.streamIdx) + 1
  ...
  c.yAxis = d3.axisLeft(c.y)
    ...
    .tickFormat(i => {
      ...
      return i == yNumTicks - 1 ? 'Lgt' : i == 0 ? 'Emb' : 'L' + i
  ...
  drawLinks(links, allCtx.allLinks, 0, 'rgba(0,0,0,.15)')
