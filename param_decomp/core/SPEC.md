...
| **delta component** | the extra maskable channel carrying `x @ (W − V@U)`; its source is the explicit per-site `delta` tensor (AMENDED 2026-08-27) | the faithfulness loss (same residual, different role) |
...
| C | 24576 (bench: 8192); the delta component is always built → every site source carries its own `delta` tensor |
...
| `sources[s]` | `{components: [source_shape dims, C]` — expert-blocked sites `[source_shape dims, E, c_per_expert]`, expert-major — `delta: [source_shape dims]}` per site (§6 SCOPE; ★ components `[1, T, C]`, delta `[1, T]`) (AMENDED 2026-08-27) | SRC_STEP ascent | yes (+ SRC_STEP moments where the SRC_STEP keeps state — `adam`; `sgd` persists the sources alone) |
...
```
def make_masks(ci_lower_s[B,T,C], source_s{components[B,T,C], delta[B,T]}) -> (mask, delta_mask):
    mask       = ci_lower_s + (1 − ci_lower_s) * source_s.components   # expert-blocked components read flat, expert-major
    delta_mask = source_s.delta                # delta source raw: NO ci interpolation     (S1)
...
def imp_min_terms(ci_upper, gamma, reference_datapoint_count, normalize_at_one):          (S7)
    scale = (1 + gamma²) if normalize_at_one else 1
    for s:                                                                               (S8,S9)
        # ci = ci_upper_s[b,t,c]
        per_component_sums[c] = Σ_{b,t} scale·ci² / (ci² + gamma²)
...
def stochastic_recon_loss(components, ci_lower, residual, clean_output):
    ...
    for routes in SAMPLE_ROUTING(key, [B,T]):            # fresh per step                 (R1,S11)
        masks, delta_masks = make_masks(ci_lower_s, source_s ~ U[0,1] per channel)  ∀ s ∈ sites
...
**AMENDED 2026-06-22**: the abstract contract is UNCHANGED — `CI_fn:
...
| S18 | Each training step consumes a fresh target input batch (a pure function of `(seed, step)` for O(1) resume); the model embeds it internally where applicable. **AMENDED 2026-06-24**: removed the prefix harvest (residual-start) — there is no separate prefix forward; clean and masked forwards take the token batch directly. **AMENDED 2026-07-30:** the unified `clean_forward` / `masked_forward` names replace the former `clean_output` / `read_activations` / `masked_output` method grid; input semantics are unchanged. |
...
| S34a | `pooled_mixed_persistent_stochastic(state_key, pool_size, adv_fraction)` (`MergedStochasticSubsetPooledPPGDReconLossConfig` / `PersistentSourcePool`): the persistent bundle stores `pool_size` rows per site instead of one source per `(batch, position)`. Row `i` across every site is ONE cross-site adversarial particle. Each step draws one row with replacement per batch element and broadcasts it over that element's positions; the SAME sampled row indices are used by all warmup ascents, the main forward, and S14′'s source-gradient retake. Gather transposes scatter-add document gradients onto selected rows. Pool rows replicate over data axes and component columns retain their ordinary TP placement, so source storage is `pool_size·Σ_site(C_site+1)` rather than `batch·positions·Σ_site(C_site+1)`. The merged stochastic/adversarial selection and routing then follow S34 unchanged. The selected 4L recipe uses `pool_size=2064` and authors source LR `0.02` explicitly (2× the former dense-source schedule); there is no hidden LR multiplier. |
| S35 | Optional per-term hidden-activation reconstruction (#94): any recon loss config may carry `hidden_acts_reconstruction: {coeff, points}`, adding `coeff · mean_points(relative squared error at the named activations)` to THAT term's masked-vs-clean comparison on every draw. It shares the term's outer-objective forwards; their capture keys include the named points. `points` name activations in the TARGET's own tap vocabulary and have NO default (which internal activations matter is an experiment question). Per-point normalization is by the clean activation's own squared scale. Logged per point under `loss/<term>/hidden_acts_reconstruction/<point>` plus the term aggregate. Eval probes admit `coeff: 0.0` as the measurement-only arm: the hidden errors are logged while the adversarial masks and total remain exactly output-only; training terms refuse zero. S14′'s default `adversary_objective: e2e` excludes hidden-activation reconstruction from source ascents only; explicit `term` mode makes the adversary ascend the complete loss. The `hidden_acts_reconstruction` option is TARGET-PASS-ONLY: a tPD non-target term refuses it (T5). **Added 2026-08-05** — this row documents shipped #94 behavior whose code already cited S35; **amended 2026-08-17** for S14′'s source-only objective split. |
...
| `MASK_SOURCE` | `stochastic` (fresh U[0,1]/Bernoulli per draw) · `constant(v)` (`v=0` CI-masked, `v=1` unmasked; no delta path) · `fresh_pgd(init, n_steps, step_size, source_shape)` · `persistent(state_key)` · `mixed_persistent_stochastic(state_key, adv_fraction)` (per-BATCH-ELEMENT Bernoulli family selection; S34) · `pooled_mixed_persistent_stochastic(state_key, pool_size, adv_fraction)` (one cross-site particle per document, broadcast over positions; S34a) | ★ pooled mixed stochastic + persistent |
...
| `SRC_STEP` | `adam(β₁,β₂,ε)` with bias correction; `sgd` (`sources += lr·grad`, stateless); `momentum_sgd(μ)` (`v = μ·v + grad; sources += lr·v` — ONE float velocity buffer, bf16 under 16-bit source storage; the EMA forms in fp32 and rounds ONCE into the velocity dtype, so the store rounding is the only 16-bit seam and `μ` itself is never rounded) (AMENDED 2026-08-27: `bsc` stores no Adam moments; momentum SGD is the reference SRC_STEP). Source VALUES may store as uint16 unit-interval fixed-point (`u/65535`): ascents differentiate/update the fp32 view and store back with STOCHASTIC rounding (unbiased — round-to-nearest stalls every sub-half-step update; saturation composes with PROJ); float storage is bit-untouched by the seam. `sign` (`sources += lr·sign(grad)`) | ★ adam(.5, .99, 1e-8) |
...
| delta component | on (each site source carries a `delta` tensor) · off (no delta path, no delta mask) | ★ on |
...
| N3 | `kl_per_position` (softmaxes + KL sum) and the imp-min reduction are fp32; the smooth-L0 per-value penalty computes on `ci` cast to fp32 first (every per-value-penalty arm in `losses.py` — `_site_frequencies`, `_narrow_site_frequencies`, `_site_activity` — casts before calling `per_value_penalty`; the narrow arm's global scatter is `components.narrow_component_sums`). Loss scalars fp32. **Gradient accumulation (the sharded spelling, stated 2026-09-02):** the single-process torch oracle accumulated gradients in fp32; the placed JAX trainer accumulates the V/U and CI-weight gradients in the COMPUTE dtype. Masters are cast to bf16 BEFORE the once-per-step entry gather (`model.prepare_compute_weights` → `placement.materialize_reduced_weights`), so the resident stacks — and, since a cotangent takes its primal's dtype, their cotangents — are bf16: the sum of the recon terms' (and draws') contributions into the resident cotangent is a bf16 accumulation, and the deferred exit reduce-scatter sums the data ranks' bf16 partials on a bf16 wire; the master gradient is fp32 only from the cast's transpose onward (the faithfulness and nonlinearity direct grads are fp32 throughout). Pinned by `tests/targets/test_qwen36_dtype_census.py`; neither per-term fp32 pullbacks nor fp32 resident cotangents are part of this implementation. **eval cast point (accepted seam):** `CEandKLLosses` — torch computes the eval softmax under bf16 autocast (`param_decomp/eval_metrics/ce_and_kl_losses.py:91-176` at the `torch-oracle` tag), JAX casts to fp32 before `log_softmax` (`experiments/lm/eval.py::next_token_cross_entropy`, `_row_masked_kl`, `_row_masked_cross_entropy`) — a bf16-vs-fp32 input cast-point divergence, not a direction/reduction divergence; the expected `kl_<v>` delta on a fixed batch is the bf16-rounding floor (rel ≪ 1e-2), accepted rather than bounded by a fixture (cf. E17). **AMENDED 2026-08-27**: the former torch-vs-JAX imp-min cast seam — torch's bf16 `(ci + eps)**p` intermediate vs JAX's fp32-first cast — retired with the `L_p` penalty; its bounding fixture (`test_imp_min_bf16_seam.py`) is deleted. |
...
| R4 | A draw is generated in its CONSUMER's dtype (`linear_plan.uniform_like` draws in the reference's dtype; every mask source, delta channel, and source init inherits the compute dtype of the value it masks or initializes). JAX generates a bf16 uniform from 16 random bits and an fp32 uniform from 32, so the two streams from ONE key are unrelated samples, not roundings of each other: a bf16 run and an fp32 run at the same seed score different mask samples and are different experiments. A parity comparison across compute dtypes must draw once, in one dtype, and cast the draw to each arm's compute dtype; otherwise the comparison measures sample noise, not numerics. |
...
| D4 | Validation property: with global batch + seed fixed, the metric trajectory is invariant to device count up to floating-point reassociation (cross-shard reduction order; observed rel ≤ ~1e-5 on the tiny-target harness, `param_decomp/targets/invariance_check.py`). JAX's counter-based RNG makes even the stochastic draws identical across layouts. **AMENDED 2026-07-15** (owner-partitioned persistence): the trainable V/U masters (and their optimizer moments) persist as same-shape STACKS — one `(Vs [g, d_in, C], Us [g, C, d_out])` pair per shape group (`components.ComponentStacks.stacks` + static `site_slots`), placed by the hybrid HSDP rule: stack axis ÷`replicate` (whole matrices owned per node-group — zero cross-node weight collectives per step; muon NS node-local), matrix d dims ÷`fsdp`, C ÷`tp` (total ÷N, same memory as the retired per-site intra-matrix ZeRO-1); a group whose stack length does not tile `replicate` falls back per-group to intra-matrix data sharding behind the stack axis. Same math, different layout — covered by THIS invariant's reassociation tolerance (re-validated on the harness at 4 sim devices). Checkpoint trees change shape (2·n_shapes stack leaves, not 2·n_sites): pre-change checkpoints need a one-off layout migration to restore at tip. Muon leaf labeling: the V/U tree is now all-3D, so both groups use `stacked_muon_dimension_numbers` (optax's default 2D rule would silently Adam every V/U leaf). **AMENDED 2026-07-21**: the non-tiling-group fallback is config-OPT-IN, not automatic — enabled by the `owner+zero1` preset (or an explicit table's declared `components.optimizer_state_fallback` row); strict `owner` errors on a group whose stack length does not tile `replicate`. **AMENDED 2026-07-21** (same day): the sharding preset/table is a BIDIRECTIONAL claim, checked at config build — the per-group persist-vs-zero1 assignment is resolved once, at `PlacementRules` construction from the run's resolved site set and its config-implied mesh (this fires at config construction before training); a declared `components.optimizer_state_fallback` row that no shape group takes is equally an error, and the trainer boundary only validates the received assignment against the arrays it holds — it never re-decides. **AMENDED 2026-08-18** (strict placement): the per-group fallback is REMOVED entirely — the `owner+zero1` preset and the explicit-table fallback rows (`optimizer_state_fallback`, `faithfulness_weights_fallback`, `faithfulness_deltas_fallback`) no longer exist, so mixed per-group placement is unrepresentable and the bidirectional claim is vacuous (nothing left to claim). One set of component rows places every shape group; a group whose stack length does not tile a stack-sharded row REFUSES at config build, the refusal naming the non-tiling groups, their stack lengths, the sharded extent, and the remedies (a tiling mesh, or a placement with no stack sharding). `zero1`'s faithfulness rows are now its intra-matrix master layout for ALL groups (the weights transition is the identity; previously tiling groups took the stack-preferring pair) — same math, layout covered by this invariant's reassociation tolerance. **AMENDED 2026-09-01** (persist-stack padding): the stack-cut refusal is replaced by explicit PADDING — construction pads each group's persist stack to the next common multiple of the stack-sharding component rows' extents (`GroupCensus.stack_pad`, an enumerated per-group fact carried by the census and mirrored on `ComponentStacks.stack_pads`; never shape-inferred). Pad slots are trailing all-zero (V, U) stack entries living ONLY in the persist layer (fp32 masters + optimizer moments) and on the faithfulness lane (whose rows ARE the master layout): the entry gather strips them (`placement.component_stacks_to_compute_weights` — compute never sees pads; the boundary's transpose scatters real grads into the real slots and exact zeros into pads), the faithfulness lane rides them as exact zeros (targets extend the frozen stack with zero matrices so pad deltas are exactly 0; `make_faithfulness_loss` keeps the site mean over REAL sites), and wd=0 (N1/S20) + exactly-zero gradients keep pads at zero through both adamw and stacked muon (NS on a zero matrix is zero; the pad-inert step is pinned by test). Muon's canonical NS stack length is the PADDED length (`GroupCensus.ns_stack_len`). Real-stack trajectories are unchanged — bit-identical to the unpadded layout at a fixed world (pinned by test), and across worlds this invariant's reassociation tolerance applies as before. Matrix-axis tiling failures and the muon staging claim still refuse; checkpoints persist the PADDED stacks, so a consumer re-placing a padded run must resolve the same pad counts. **AMENDED 2026-09-03** (CI-fn chunk stack): the chunkwise CI fns' `[n_chunks, ...]` persist stacks take the SAME padding — one `StackCensus` (real length + pad) resolved in `ci_fn.resolve_ci_placement` over every row the stacked leaves rest at (`CIFnRows.chunk_persist_rows`; the MoE families' masters too), carried on `CIFnPlacement.chunks` and mirrored on the fn's static `stack_pad`; every leaf of the stacked chunk module pads (`pad_ci_fn`), `materialize_ci_compute_weights` strips the pads at the entry so the chunk scan runs only real chunks, and the CI muon staging claims tile the PADDED chunk count. **AMENDED 2026-09-03** (real-slot entry gather): both entries strip the pads BEFORE the cross-`data` gather — a padded stack hops through its entry waypoint (`placement.padded_entry_waypoint`: the compute layout with the stack cut re-parked minor on the leaf's last axis, an all-to-all of one padded slot per device), strips device-local there, and gathers real slots only, so the entry all-gather and its reduce-scatter transpose carry `stack_len`, never `padded_stack_len`. Same values (real-slot residents and gradients bit-identical to the gather-then-strip spelling, pinned by test); the waypoint's minor-axis tiling is a construction-time refusal like every matrix-axis tiling. The chunk-stack census resolves at the config-build gate too, so an untiled CI-fn stack refuses (or pads) pre-submit. |
...
| T10 | The TARGET pass treats the delta exactly as plain VPD does: stochastic terms draw a `U[0,1]` delta source, adversarial terms carry their source's delta channel, ascended and projected over `[0,1]` (S1/S13/S15). **RESOLVED 2026-08-27** (consistent with the tPD paper's spelling — arXiv 2607.13047, Method details): on target data the delta mask is part of the adversarial argmax — "adversarially ablated on target batches (each between 0 and 100%) … forcing the target outputs to be reconstructed from the subcomponents alone" — with the auxiliary stochastic arm drawn "as in Bushnaq et al. (2025)". Pinning the delta OFF on-target is NOT the paper's procedure and remains unimplemented; the paper's only delta-off arm is the CSS unmasked-recon term (T4's exception). Why free rather than pinned: on target data the paper enforces "components alone reconstruct" as the WORST CASE of a continuous ablation family (the adversary drives `m_Δ` toward 0 wherever that hurts most), and hybrids of original and ablated matrices stay covered, which a hard `m_Δ = 0` pin would surrender. |
...
(the targeted-decomposition formulation described in that paper).
Open item: whether T3's faithfulness exclusion is definitional or a regime choice
awaits the algorithm's author-definition; it is currently enforced structurally
