...
Note, the default batch sizes are 256 for harvest and 64 for dataset attributions, which work well for models like `ss_llama_simple_mlp-2L-wide`.

Also, the `--n_gpus` argument should be configured to the max you have available.
...
```bash
spd-attributions wandb:spd/runs/abc123
```
