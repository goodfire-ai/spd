...
class CodebaseIntegrator:
    ...
    def clone_or_download(self, source: str, target_name: str | None = None) -> Path:
    ...
    def analyze_codebase(self, codebase_path: Path) -> dict[str, Any]:
        ...
        try:
            if (codebase_path / "package.json").exists():
                with open(codebase_path / "package.json") as f:
                    pkg = json.load(f)
        ...
        try:
            if (codebase_path / "requirements.txt").exists():
                with open(codebase_path / "requirements.txt") as f:
...
"""
    ...
    def save_integration(
        ...
        print("📄 CLAUDE.md generated for easy reference")

    def integrate(self, source: str, name: str | None = None) -> dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print("4. Run analysis: /integrate-external-codebase follow-up")
        ...
        print("   - For private repos, set up authentication first")
        print("   - Try with --name flag for custom naming")
        return 1
